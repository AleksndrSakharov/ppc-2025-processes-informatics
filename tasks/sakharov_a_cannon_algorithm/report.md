...
### 4.2. Реализация функции MyBcast
...
- `mpi/include/ops_mpi.hpp` — объявление `MyBcast` и класса задачи.
...
## 8. Заключение
В ходе работы была успешно реализована функция `MyBcast`, выполняющая коллективную рассылку данных с использованием точечных обменов `MPI_Send` и `MPI_Recv`.
