...
bool SparseMatrixMultiplicationMPI::RunImpl() {
  ...
  local_result.row_ptr.resize(static_cast<size_t>(my_num_rows) + 1);
  ...
  for (int local_i = 0; local_i < my_num_rows; ++local_i) {
    ...
    for (int j = 0; j < mat_b_.cols; ++j) {
      double sum = 0.0;

      int a_start = mat_a_.row_ptr[global_i];
      int a_end = mat_a_.row_ptr[global_i + 1];
      ...
      if (std::abs(sum) > 1e-12) {
        ...
        local_result.col_indices.push_back(j);
      }
    }
